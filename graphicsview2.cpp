...
GraphicsView2::GraphicsView2(QWidget *parent) {
    isPanning = false;
    lastMousePos = QPoint();
    cellSize = 30;
    vizData = new VizData();
...
void GraphicsView2::updateRasterData() {
...
void GraphicsView2::updateRasterDataPixmap(){
...
void GraphicsView2::updateRasterDataFreq(const QString& aa_sequence, int month,
                                            double thresholdMin,
                                            double thresholdMax) {
    if (squareItemList.isEmpty()) {
        initSquareItems();
    ...
    if(aa_sequence.isEmpty()){
        qDebug() << "[GraphicsView2] Amino acid sequence is empty!";
        return;
    }
    ...
    setOverlayText(QString("KENYA\n%1\nPercent of resistant genotypes: %2%")
                       .arg(vizData->simStartDate.addMonths(month).toString("yyyy-MM-dd"))
                       .arg(vizData->statsFrequencySummary["KNF--H1"].median[month]*100, 0, 'f', 2));
...
void GraphicsView2::updateRasterDataFreqPixmap(const QString& aa_sequence, int month) {
    if (aa_sequence.isEmpty()) {
        qDebug() << "[GraphicsView2] Amino acid sequence is empty!";
        return;
    }
    ...
    if (locationFreqMap.isEmpty()) {
        qDebug() << "[GraphicsView2] No genotype frequency found for month:" << month << ", aa_sequence:" << aa_sequence;
        return;
    }
